...
/*read the data from sd card, then store these data to the read buffer*/
            ...
            if (kStatus_Success != errorCode)
            {
                g_msc.read_write_error = 1;
                usb_echo(
                    "Read error, error = 0xx%x / %d \t Please check read request buffer size(must be less than 128 "
                    "sectors)\r\n",
                    error, errorCode);
                error = kStatus_USB_Error;
            }
...
void SDCardTest() {
	...
	for (int i=0; i<32768; i+=TEST_BLOCK_COUNT) {
	    status_t s = SD_ReadBlocks(usbDeviceMscSdcard, buffer, i, TEST_BLOCK_COUNT);
	    if (s) {
	    	PRINTF("ERR %d\n", s);
	    }
	}
